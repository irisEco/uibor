...
func (u *User) Domessage(msg string) {
	...
	if strings.Contains(msg, "rename|") { // 判断是否包含 rename | 关键字来修改用户名
		...
		for _, user := range u.server.OnlineMap {
			fmt.Println("1", user.Name, "1")
			// fmt.Println("len: ", len(user.Name))
			fmt.Println("1", info, "1")
			fmt.Println("len: ", len(info))
			// name := strings.ReplaceAll(user.Name, " ", "")
			fmt.Println(strings.TrimSpace(user.Name) == strings.TrimSpace(info))
			// if user.Name == info {
			// 	// chatter := user
		...
		chatter,ok = u.server.OnlineMap(info)
		u.server.mapLock.Unlock()
		// if ok {
...
func (u *User) Rename(msg string) {
	...
	if ok {
		...
		u.server.mapLock.Lock()
		delete(u.server.OnlineMap, u.Name)
		u.Name = strings.TrimSpace(user.Name) //去除空格
